<template>
  <div class="topic-page">
    ...
    <textarea v-model="newPostText" placeholder="Введите текст поста"></textarea>
...
export default {
  ...
  setup() {
    ...
    // Загрузка постов для этого топика
    async function fetchPosts() {
      try {
        const response = await fetch(`http://localhost:3000/posts?topic_id=${route.params.id}`);
        if (response.ok) {
